...
chatbot_cq = gr.Chatbot(value = [[None, initial_msg_cq]])
chatbot_gk = gr.Chatbot(value = [[None, initial_msg_gk]])
chatbot_lp = gr.Chatbot(value = [[None, initial_msg_lp]])

# Creating interfaces for each chatbot
chat_interface_cq = gr.ChatInterface(chat_gen_cq, chatbot=chatbot_cq).queue()
chat_interface_gk = gr.ChatInterface(chat_gen_gk, chatbot=chatbot_gk).queue()
chat_interface_lp = gr.ChatInterface(chat_gen_lp, chatbot=chatbot_lp).queue()
# demo = gr.ChatInterface(chat_gen, chatbot=chatbot).queue()

with gr.Blocks() as demo:
    with gr.Row():
        with gr.Column():
            chat_interface_cq.render()
        with gr.Column():
            chat_interface_gk.render()
        with gr.Column():
            chat_interface_lp.render()
